...
const Royalties = (props) => {
    ...
    function addToRoyaltyList() {
        ...
        if (rateTypeSelected === "percent") {
            if (Validator.validateRoyalty_Add(
                ...
                setNotify({isOpen:true, message:'Entry added to royalty distribution list.', type:'success'})
               ...
               /* Post validated data to Panel array "royaltieslist" (for Perctage) */ 
                props.Panels.royalties.royaltieslist = [...props.Panels.royalties.royaltieslist, 
                    {
                        id: idCount,
                        description: document.getElementById("RoyaltyDescription").value,
                        rateType: document.getElementById("RoyaltyRateType").value,
                        amount: document.getElementById("RoyaltyRatePercent").value,
                        denom: '',
                        address: document.getElementById("RoyaltyAddress").value
                    }
                ]
                ...
                props.Panels.royalties.royaltieslist = [...props.Panels.royalties.royaltieslist, 
                    {
                        ...
                        description: document.getElementById("RoyaltyDescription").value,
                        rateType: document.getElementById("RoyaltyRateType").value,
                        amount: document.getElementById("RoyaltyFlatAmount").value,
                        denom: document.getElementById("RoyaltyRateDenom").value,
                        address: document.getElementById("RoyaltyAddress").value
                    }
                ]
                props.Panels.royalties.isValidated = true
    ...
    function removeFromList(royalty) {
        props.Panels.royalties.royaltieslist = _.reject(props.Panels.royalties.royaltieslist, royalty)
        if (props.Panels.royalties.royaltieslist.length <= 0) {props.Panels.royalties.isValidated = false} //pull validation if no addresses left
   
        
        setNotify({isOpen:true, message: 'Entry removed from royalty address list.', type:'warning'})
    }
    ...
    return (
                <div id="royalties-panel" className="row">
                    ...
                    <div id="inner-content-box" 
                        className={props.Panels.royalties.highlight?
                    ...
                    >
                        ...
                        <p className="h6">Fees paid to the author, owner of the NFT. By percentage or flat fee.</p>
                                ...
                                <div>
                                    ...
                                    <div className="form-group my-2">
                                        <div className="form-group">
                                            <label htmlFor="RoyaltyDescription" className="lead">Royalty Description</label>
                                            <input type="text" className="form-control" id="RoyaltyDescription" placeholder="Creator's Share" />
                                            <label htmlFor="RoyaltyDescription" className="small">*Optional - Information is solely for organizational use.</label>
                                        </div>
                                        
                                        {/* Distribution type assignment for form by dropdown */}
                                        <label className="my-1 mr-2 lead" htmlFor="RoyaltyRateType">Distribution Type</label>
                                        <select 
                                            id="RoyaltyRateType"
                                        ...
                                        <div className="row light-opacity rounded-lg p-3 mt-3">
                                            ...
                                            {rateTypeSelected === 'percent' && (
                                                <div>                                                                                    
                                                    <label htmlFor="RoyaltyRatePercent" className="row lead">% fee of sale price</label>
                                                    <div className="row">
                                                        <input type="text" className="form-control col-5" id="RoyaltyRatePercent" placeholder="5%" />
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {rateTypeSelected === 'flat' && (
                                                <div className="row">
                                                    <div className="col pl-0">
                                                        <label htmlFor="RoyaltyFlatAmount" className="lead">Flat fee</label>
                                                        <input type="text" id="RoyaltyFlatAmount" className="form-control col" placeholder="$50.00" />
                                                    </div>
                                                    <div className="col">
                                                        <label htmlFor="RoyaltyRateDenom" className="lead">Currency type</label>
                                                        ...
                                                        <select id="RoyaltyRateDenom"className="custom-select my-1 mr-sm-2">
                                        ...
                                        <div className="form-group my-5">
                                            ...
                                            <label htmlFor="RoyaltyAddress" className="row lead">Alphanumeric Terra Address</label>
                                            <div className="row">
                                                <input type="text" className="form-control col" id="RoyaltyAddress" placeholder="terra14lxhx09fyemu9lw46c9m9jk63cg6u8wdc8pdu4" />
                                                <button 
                                                    type="button" 
                                    ...
                                    {props.Panels.royalties.royaltieslist.map((royalty) => (
                                        <div key={royalty.id} className="row mid-opacity rounded-lg text-dark small p-3 mb-2">
                                            <div className="col text-left m-0">
                                                {royalty.description? <p className="lead">{royalty.description}</p> : null}
                                                {   //only show if ratetype is percentage
                                                    royalty.rateType === "percent" && (<p><span className="small">Amount: </span>{royalty.amount}% </p>) 
                                                }
                                                {   //only show if ratetype is flat
                                                    royalty.rateType === "flat" && (<p><span className="small">Amount: </span>{royalty.amount} {royalty.denom}</p>) 
                                                }
                                                <p><span className="small">To: </span>{royalty.address}</p>
                                            </div>
                                            <a className="col-2  text-danger" onClick={() => removeFromList(royalty)}>Remove</a>
                                        </div>
                                    ))
                                    }
                                </div>
