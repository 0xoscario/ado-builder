...
const Blacklist = (props) => {
    ...
    function addToBlacklist() {
        ...
        // Validate form field
        if (Validator.validateBlacklist_Add(document.getElementById("blacklistAddAddress").value)) {
            setNotify({isOpen:true, message:document.getElementById("blacklistAddAddress").value + ' added to blacklist.', type:'success'})
        
            props.Panels.blacklist.toBlacklist = [{id: props.Panels.blacklist.toBlacklist.length + 1, address:document.getElementById("blacklistAddAddress").value}, ...props.Panels.blacklist.toBlacklist]
    ...
    function removeFromBlacklist(address) {
        props.Panels.blacklist.toBlacklist = _.reject(props.Panels.blacklist.toBlacklist, {'address':address})
        if (props.Panels.blacklist.toBlacklist.length <= 0) {props.Panels.blacklist.isValidated = false} //pull validation if no addresses left
    ...
    return (
        <div id="blacklist-panel" className="row">
            ...
            className={props.Panels.blacklist.highlight?
            ...
            >
                ...
                <p className="h6">A list of addresses prohibited from interaction with your NFT.</p>
                        ...
                        <div>
                            ...
                            {props.Panels.blacklist.toBlacklist.map((tAddress) => (
                                <div key={tAddress.id} className="row mid-opacity rounded-lg text-dark small p-3 mb-2">
                                    ...
                                    <a className="col-2  text-danger" onClick={() => removeFromBlacklist(tAddress.address)}>Remove</a>
                                </div>
                                ))
                            }
